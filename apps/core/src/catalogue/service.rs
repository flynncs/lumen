...
use crate::{
    catalogue::{CatalogueResolver, CatalogueSearch, SourceIdentity, Track, TrackId},
...
pub struct CatalogueService {
    ...
    tracks_by_source: Mutex<HashMap<SourceIdentity, TrackId>>,
}

impl CatalogueService {
    pub fn new(resolver: Arc<dyn CatalogueResolver>) -> Self {
        Self {
            resolver,
            tracks_by_source: Mutex::new(HashMap::new()),
        }
    }
        ...
        let tracks = {
            ...
            candidates
                ...
                .map(|candidate| {
                    ...
                    let track_id = tracks_by_source
                        .entry(source)
                        .or_insert_with(TrackId::new)
                        .clone();

                    Track::new(track_id, metadata)
                })
                .collect()
        };
...
mod tests {
    ...
    use super::*;
    use crate::{
        catalogue::{CatalogueCandidate, ProviderId, SearchLimit, SearchQuery, TrackMetadata},
        request::RequestId,
    };
    ...
    async fn search_assigns_stable_ids_per_source_and_uses_latest_metadata() {
        ...
        let service = CatalogueService::new(Arc::new(resolver));
